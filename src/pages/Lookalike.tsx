...
export default function Lookalike() {
  ...
  const handleSaveImage = async () => {
    ...
    try {
      ...
      const filename = `kpopstudio-${matchedIdol.member.name.en.replace(/\s+/g, '-').toLowerCase()}.png`;
          ...
          <div className="neon-border-animated glass-card rounded-[32px] md:rounded-[48px] p-1 w-full mb-10 md:mb-16 overflow-hidden max-w-4xl mx-auto">
            <div className="bg-black/80 backdrop-blur-3xl rounded-[30px] md:rounded-[46px] p-6 md:p-10 lg:p-12 flex flex-col items-center relative z-10">
              ...
              <div className="flex flex-col md:flex-row items-center justify-center gap-6 md:gap-10 w-full mb-10 md:mb-14">
                ...
                {matchedIdol && (
                  <div className="relative">
                    <div className="w-48 h-48 xs:w-56 xs:h-56 md:w-60 md:h-60 lg:w-64 lg:h-64 rounded-[28px] md:rounded-[32px] overflow-hidden shadow-2xl border-2 border-neon-blue neon-shadow-blue relative flex-shrink-0">
                      ...
                      <div className="absolute inset-0 flex items-end justify-center pb-3">
                        <span className="text-white font-mono text-[10px] uppercase font-black tracking-widest opacity-0 bg-black/60 px-2 py-1 rounded-lg">
              ...
              <div className="w-full text-center space-y-8 md:space-y-10">
                ...
                {matchedIdol && (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full max-w-2xl mx-auto">
                    {[
                      ...
                      { label: t('blood_type'), value: matchedIdol.member.bloodType + (i18n.language === 'ko' ? '형' : ''), borderCls: 'border-neon-green/30', textCls: 'text-neon-green' },
                ...
                {currentTmi && (
                  <div className="w-full max-w-2xl mx-auto">
                    <div className="bg-neon-yellow/5 border border-neon-yellow/20 rounded-2xl p-5 md:p-6 text-left">
                      <div className="flex items-center gap-2 mb-3">
                        <Sparkles className="w-4 h-4 text-neon-yellow shrink-0" />
                        <p className="text-neon-yellow font-mono text-[10px] uppercase font-black tracking-widest flex-1">
                          {t('behind_story')}
                        </p>
                        {tmiArr.length > 1 && (
                          <button
                            onClick={() => setTmiIndex(i => i + 1)}
                            className="w-7 h-7 rounded-lg bg-neon-yellow/10 hover:bg-neon-yellow/25 border border-neon-yellow/30 hover:border-neon-yellow/60 flex items-center justify-center transition-all active:scale-90"
                            title="다음 TMI"
                          >
                            <RefreshCw className="w-3.5 h-3.5 text-neon-yellow" />
                          </button>
                        )}
                      </div>
